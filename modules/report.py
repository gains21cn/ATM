...
"""
账单生成模块：
从report_bill中便利所有流水记录，获取所有卡号
将卡号存放到列表中
对列表进行集合类型转换
根据集合中的唯一的卡号信息，每个卡号生成一个文件 cardno_startdate_enddate报表文件,文件中存放字典信息对账单
将对账的费用统计信息接入一个统计文件 ,包括字段：卡号、对应详单文件名、账单日期、还款日期、应还款金额、已还款金额、
{"detail":{[],[]}
 "
"""
